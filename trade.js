...
(async function main() {
  ...
  bot.sendMessage(
    process.env.GROUPID,
    `Start trade *${pairs[0]}* with balance *${balance} ${tokenBuy}*, volumn per order *${moneyPerOrder} ${tokenBuy}*`,
    { parse_mode: "Markdown" }
  );
    ...
    binance.websockets.chart(
      ...
      async (symbol, interval, chart) => {
        ...
        if (boughtAmount !== null) {
          ...
          bot.sendMessage(
            process.env.GROUPID,
            `Sell market setup, balance *${balance} ${tokenBuy}*`,
            { parse_mode: "Markdown" }
          );
        }
        ...
        if (lastClose > lastOpen && +tickBefore.close > +tickBefore.open) {
          ...
          bot.sendMessage(
            process.env.GROUPID,
            `Buy setup ${symbol}, amount *${boughtAmount}*, price *${lastClose.toFixed(
              2
